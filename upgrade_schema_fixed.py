#!/usr/bin/env python3
"""
Upgrade database schema to professional Investment Advisor structure (Fixed version)
- Keep existing transactions table but add type column
- Create new goals table
- Update existing risk_profile table to professional standards
...
def upgrade_investment_schema():
    try:
        ...
        # 3. Update existing risk_profile table to professional standards
        print("⚖️  Updating risk_profile table...")
        
        # Check existing columns in risk_profile
        ...
        for column, datatype in new_columns.items():
            if column not in existing_columns:
                try:
                    cursor.execute(f'ALTER TABLE risk_profile ADD COLUMN {column} {datatype}')
                    print(f"   ✅ Added {column} column")
                except sqlite3.OperationalError as e:
                    print(f"   ⚠️  Could not add {column}: {e}")
        
        # Update existing risk profile or add new one
        cursor.execute('SELECT COUNT(*) FROM risk_profile')
        profile_count = cursor.fetchone()[0]
        
        if profile_count == 0:
            # Add professional risk profile
            sample_risk_profile = {
                'age': 35,
            ...
            print("   ✅ Added professional risk profile")
        else:
            # Update existing profile to ensure risk_tolerance uses proper values
            cursor.execute('''
            UPDATE risk_profile 
            SET risk_tolerance = CASE 
                WHEN risk_tolerance LIKE '%Low%' OR risk_tolerance LIKE '%Conservative%' THEN 'Conservative'
                WHEN risk_tolerance LIKE '%High%' OR risk_tolerance LIKE '%Aggressive%' THEN 'Aggressive'
                ELSE 'Balanced'
            END
            ''')
            print("   ✅ Updated existing risk profile")
        ...
        if profile:
            print(f"\n⚖️  Risk Profile:")
            if len(profile) > 1 and profile[1]: print(f"   Investor: {profile[1]}")
            if len(profile) > 2: print(f"   Age: {profile[2]}")
            if len(profile) > 3: print(f"   Risk Tolerance: {profile[3]}")
            if len(profile) > 4: print(f"   Investment Horizon: {profile[4]} years")
            if len(profile) > 5: print(f"   Annual Income: ${profile[5]:,.2f}")
            if len(profile) > 6: print(f"   Dependents: {profile[6]}")
            if len(profile) > 7: print(f"   Experience: {profile[7]}")
            if len(profile) > 8: print(f"   Knowledge: {profile[8]}")
        
        # Investment recommendations based on risk profile
        risk_tolerance = profile[3] if profile and len(profile) > 3 else 'Balanced'
